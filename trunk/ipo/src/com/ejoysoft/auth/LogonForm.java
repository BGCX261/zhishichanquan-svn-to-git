...
import com.ejoysoft.wlanyz.system.*;
...
public final class LogonForm {
    ...
    public int userPwdAuth(String tUserId) {
        ...
        try {
            ...
            String strSql = "SELECT strId,strUserId,strPWD,strName,intError,intState,intType,strEmail,strMsnQQ,strCssType " +
            		"FROM sys_user WHERE strUserId='" + tUserId + "' ";
...
//                    value = -1;
                ...
                globa.db.executeUpdate("UPDATE sys_user SET intLoginNum=intLoginNum+1,intError=0,dLatestLoginTime= convert(datetime,'"+Format.getDateTime()+"',121) where strUserId = '" + tUserId + "'");
    ...
    private int authMac(String CaNO) throws SQLException {
        int value = -1;
        boolean isNo = false;
        ResultSet rs = globa.db.executeQuery("SELECT * from sys_user where strCaNO='" + CaNO + "'");
        boolean isRes = rs.next();
        if (!isRes) {
            isNo = true;
            String tUserId = Format.removeNull(this.getPassword2());
            rs = globa.db.executeQuery("SELECT * from sys_user WHERE strUserId='" + tUserId + "' ");
            isRes = rs.next();
        }
        if (isRes) { // 正确
            //用户相关信息
            ...
            userSession.setStrCssType(rs.getString("strCssType"));
            //更新证书序列号
            if (isNo) globa.db.executeUpdate("UPDATE sys_user  SET strCaNO='" + CaNO + "'  where strUserId = '" + userId + "'");
            //更新登录成功次数和时间
            globa.db.executeUpdate("UPDATE sys_user SET intLoginNum=intLoginNum+1,intError=0,dLatestLoginTime=str_to_date('"+Format.getDateTime()+"','%Y-%m-%d %T')  where strUserId = '" + userId + "'");
            //用户所属的单位id
            User user = new User(globa);
    ...
    public boolean doModiPass() {
        try {
            error = null;
            String sql = "UPDATE sys_user SET strPWD=? WHERE  strUserId=? AND strPWD=?";
    ...
    public boolean doSetPwd(String strid) {
        try {
            String sql = "UPDATE sys_user SET strPWD=?,intState=? ,intError=0 WHERE  strUserId=? ";
            globa.db.prepareStatement(sql);
    ...
    private void loadRight() {
        ...
        try {
            if (userSession.getStrUserId().equals(Constants.Administrator)) {
                ...
                String strSql = "SELECT strRoleId FROM sys_userRight WHERE strUserId='" + userSession.getStrId() + "' AND intType=0";
                rs = globa.db.executeQuery(strSql);
                while (rs.next()) {
