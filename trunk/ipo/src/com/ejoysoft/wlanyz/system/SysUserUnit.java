...
public class SysUserUnit {
    ...
    public static void init(String tWhere) {
        try {
        	//query db
            String sql = "SELECT * FROM sys_unit  " + tWhere + "  ORDER BY strParentId";
            ...
            while (ite.hasNext()) {
                ...
                if (ug.getStrParentId().equals("")) {
                	...
                	if(userUnits.get(ug.getStrParentId())!= null)
...
//        // this.rootUserGroups重新排序
    ...
    public static void validateDelId(String id) throws UserUnitIdException, SQLException {
        ...
        {
            throw new UserUnitIdException("不能删除还有下级部门的上级部门", "请先删除下级部门");
        }
        //验证是否有用�?
        String sql = "SELECT * FROM sys_user WHERE strUnitId='" + id + "'";
        ...
        if (rs.next()) {
            ...
            stmt.close();
            con.close();
            throw new UserUnitIdException("不能删除还有用户的部门", "请先删除部门内的用户");
        }
        rs.close();
        stmt.close();
        con.close();
    ...
    /**
     * 判断strChildId代表的部门是否归strParentId代表的部门管�?
     * @param strParentId
     * @param strChildId
     * @return
     */
