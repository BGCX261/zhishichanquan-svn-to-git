...
public class User {
    ...
    String strTableName = "sys_user";
    String strTableName2 = "sys_unituser";
    String strTableName3 = "sys_userright";
    ...
    public boolean addUnitUser(String strUserId, String[] arryUnitId) {
        ...
        try {
            //����Ϣд����ݿ�?
            strSql = "INSERT INTO " + strTableName2 + "  (strUserId, strUnitId, intSort, strCreator, dtCreatTime) VALUES (?,?,?,?,str_to_date(?,'%Y-%m-%d %T'))";
  ...
  //获得制定用户所属的单位
    public HashMap unitUser(String tWhere) {
        HashMap result = new HashMap();
      System.err.println(tWhere+"++++++++++_________________----------------------------==========");
        String strSql = "SELECT b.strUserId,b.strId FROM sys_unitUser a LEFT JOIN sys_user b ON a.strUserId=b.strUserId   ".concat(tWhere);
        try {
            ResultSet rs = db.executeQuery(strSql);
