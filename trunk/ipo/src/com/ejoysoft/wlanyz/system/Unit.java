    ...
    public boolean add() {
        ...
        try {
            ...
            strSql = "INSERT INTO " + strTableName + " (strId, strUnitName, strEasyName, strUnitCode, " +
            		"strParentId,strUnitPhone,strUnitAddress,intLevel,intSort,strRemark) VALUES (?,?,?,?,?,?,?,?,?,?) ";
            ...
            db.setInt(8, intLevel);
            db.setInt(9, intSort);
            db.setString(10, strRemark);
//            db.setString(17, globa.loginName);
//            db.setString(18, Format.getDateTime());
//            db.setString(19, strOrgType);
//            db.setString(20, strAFlag); 
            if (db.executeUpdate() > 0) {
                Globa.logger0("增加部门成功！", globa.loginName, globa.loginIp, strSql, "部门管理", globa.userSession.getStrDepart());
                return true;
            } else
                return false;
        } catch (Exception e) {
    ...
    public String creatUnitCode(String strParentId) {
        ...
        try {
            if (strParentId.equals("")) {
                //û���ϼ���λ
                rs = db.executeQuery("SELECT  strUnitCode  FROM " + strTableName + " WHERE strParentId='' or strParentId is null ORDER BY strUnitCode DESC");
                if (rs != null && rs.next()) {
                    tStrUnitCode = String.valueOf(Integer.parseInt(rs.getString("strUnitCode")) + 1);
                    if(tStrUnitCode.length()==1)
                    	tStrUnitCode = "0" + tStrUnitCode;
                } else
                    tStrUnitCode = "10";
            } else {
                //���ϼ���λ
                Unit ut0 = show(" where strId='" + strParentId + "'");
                rs = db.executeQuery("SELECT   strUnitCode  FROM " + strTableName + " WHERE  strParentId='" + strParentId + "' and strUnitCode like '" + ut0.getStrUnitCode() + "__' and row_number() >0 and row_number()<=1 ORDER BY strUnitCode DESC");
                if (rs != null && rs.next()) {
    ...
    public int creatSort(String strParentId) {
        ResultSet rs = null;
        int tIntSort = 0;
        String sql = "SELECT Max(intSort)  FROM " + strTableName + " WHERE  strParentId='" + strParentId + "'";
        try {
    ...
    public boolean delete(String where) {
        try {
            ...
            Globa.logger0("删除部门成功！", globa.loginName, globa.loginIp, sql,"部门管理", globa.userSession.getStrDepart());
            this.setResultDes("操作");
            return true;
        } catch (Exception ee) {
            ee.printStackTrace();
            this.setResultDes("操作");
    ...
    public boolean update(String tStrId) {
        try {
            ...
            String strSql = "UPDATE  " + strTableName + "  SET strUnitName = ?,strEasyName = ?,strUnitCode = ?," +
            		"strParentId = ?,strUnitPhone = ?,strUnitAddress = ?,intLevel = ?,intSort = ?,strRemark = ?  WHERE strId = ? ";
            if (strParentId.equals(strOldParentId)) {
                ...
                db.setString(5, strUnitPhone);
                db.setString(6, strUnitAddress);
                db.setInt(7, intLevel);
                db.setInt(8, intSort);
...
//                db.setString(19, strAFlag);
 ...
 //               db.setInt(14, creatSort(strParentId));
            ...
            Globa.logger0("修改部门信息成功！", globa.loginName, globa.loginIp, strSql,"部门管理", globa.userSession.getStrDepart());
            return true;
        } catch (Exception e) {
    ...
    public boolean updateSelf(String tStrId) {
        try {
            ...
            db.setString(10,"to_date('"+com.ejoysoft.common.Format.getDateTime()+"','yyyy-MM-dd hh24:mi:ss')");
            db.setString(11, tStrId);
            db.executeUpdate();
            Globa.logger0("修改本部门信息成功", globa.loginName, globa.loginIp, strSql, "部门管理", globa.userSession.getStrDepart());
            return true;
        } catch (Exception e) {
...
//-------------------------------------------------------------------------------------------------------
    ...
    private String strUnitName;//	部门名
    private String strEasyName;//   部门简称
    private String strUnitCode;//	办公电话
    private String strParentId;//	上级部门
    private String strDistrictId;	// ����Ͻ��ID
    private int intLevel;//	��λ����
    private int intSort;//	�����?
    private String strUnitAddress;//	部门办公地址ַ
    ...
    public void loadChildren(Vector userUnitTree) {
        ...
        for (int i = 0; i < this.children.size(); i++) {
            ...
            if (i == this.children.size()-1) {
				unit0.setLastNode(true); 
			}else {
				unit0.setLastNode(false);
			}
